...
public class Clickon : MonoBehaviour
{
    ...
    private void OnMouseDown()
    {
        ...
        if(game.TerminalState(game.BOARD)==true)
        {
            if (game.CountBlackTokens(game.BOARD) > game.CountWhiteTokens(game.BOARD))
                HFTDialog.MessageBox("END", "END OF THE GAME BLACK WINS", () => { Debug.Log("Cant insert"); });

            if (game.CountBlackTokens(game.BOARD) > game.CountWhiteTokens(game.BOARD))
                HFTDialog.MessageBox("END", "END OF THE GAME WHITE WINS", () => { Debug.Log("Cant insert"); });

            if (game.CountBlackTokens(game.BOARD) == CountWhiteTokens(game.BOARD))
                HFTDialog.MessageBox("END", "END OF THE GAME EQUALITY", () => { Debug.Log("Cant insert"); });
        ...
        if (game.current_turn == "W")
        {
            ...
            List<Move> legal = findlegalMovesCount(game.BOARD, "W");

            if (legal.Count == 0)
            {
                game.current_turn = "B";
                HFTDialog.MessageBox("Warning", "No possible moves, black turn", () => { Debug.Log("Cant insert"); });
            }
            else
            {
                int o = 0;
                for (o = 0; o < legal.Count; o++)
                    if (legal[o].i == i && legal[o].j == j)
                        break;
                if (o != legal.Count)
                {
                ...
                {
                    HFTDialog.MessageBox("Warning", "Token not inserted try again", () => { Debug.Log("Cant insert"); });
                }
            }
        }
        ...
        {
            List<Move> legal = findlegalMovesCount(game.BOARD, "B");

            if (legal.Count == 0)
            {
                game.current_turn = "W";
                HFTDialog.MessageBox("Warning", "No possible moves for black,white turn", () => { Debug.Log("Cant insert"); });
            }
            else
            {
                Move m = getBestMove(game.BOARD, game.level, "B");
